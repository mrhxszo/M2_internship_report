...
we can significantly reduce simulation time. In this section, we will briefly discuss parallelisation and give an overview of the possible strategies for parallelisation 
...
flexible as the CPU. It is designed for parallel processing of large amounts of data, making it ideal for specific tasks like matrix multiplication. Nevetheless, 
...
for each generation (or step), and each individual in the population is evaluated independently thourhg the calculation of the goodness of fit. Since one generation 
...
The initial CDSAXS algorithms contained multiple for-loops, making the parallelisation not possible. One of my first task was to replace
...
\ref{fig:cupy}. The are significant improvments between the vectorized and non vectorized code, more than an order of magnitude gain in time.
