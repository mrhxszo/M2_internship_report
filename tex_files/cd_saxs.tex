...
\color{red} Figure 10: Change the figure Intensity vs $Q_z$. Add an extra image where you show the rotation and how you convert in qz.
...
During my work-study program, we were mainly concerned with a periodic array of lines, refered to line gratings afterwards (see figure \ref{fig:isolated_line}). The cross-section of
...
\color{red} You should probably add a sentence to describe the parameters, linewidth, sidewall angles. It might help for future explantions \color{black}
...
Our analytical model predicts a perfect shape. In order to take into account defects inherent to th elithography process, 
additionals factor are introduced to account for an additional decay of scattered intensity $I(Q_{x})$.
...
CD-SAXS measurments provides experimental data. The objective of the inverse fitting is to iterate over a high number of different line profile (represented as various combination of stacked trapezoids)
...
For CD-SAXS experiment, the algorithm starts with the experimental data collected. Then, a series of in-depth line profile generated, through a set of paramters as describe earlier. 
